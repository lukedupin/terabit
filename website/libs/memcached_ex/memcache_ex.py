...
class Client(threading.local):
    ...
    # Set a key, but read the response back immediately. Read/Write
    def rw(self, key, val, time=0, min_compress_len=0):
        return self._set("rw", key, val, time, min_compress_len, False)
    ...
    def _set(self, cmd, key, val, timeout, min_compress_len=0, noreply=False):
        key = self._encode_key(key)
        if self.do_check_key:
            self.check_key(key)
        server, key = self._get_server(key)
        ...
        def _unsafe_set(depth=0):
            self._statlog(cmd)

            if cmd == 'cas' and key not in self.cas_ids:
                return self._set('set', key, val, timeout, min_compress_len,
                                 noreply)

            # Convert the information we are setting
            ...
            if cmd == 'cas':
                headers = ("%d %d %d %d"
                           % (flags, timeout, len_val, self.cas_ids[key]))
            else:
                headers = "%d %d %d" % (flags, timeout, len_val)
            fullcmd = self._encode_cmd(cmd, key, headers, noreply,
                                       b'\r\n', encoded_val)
            ...
            try:
                ...
                # Run a get value right after setting the key
                if cmd == 'rw':
                    rkey, flags, rlen, = self._expectvalue( server, raise_exception=True )

                    # Pull back a response from the memcache
                    if not rkey:
                        if depth >= 3:
                            #print("Giving up")
                            return None
                        ...
                        # Retry
                        #print(fullcmd)
                        #print("Retrying sending data: %d" % depth)
                        #print(headers)
                        time.sleep(0.01)
                        return _unsafe_set( depth + 1 )
                    try:
                        value = self._recv_value(server, flags, rlen)
                    finally:
                        server.expect(b"END", raise_exception=True)

                # Normal set response check
                else:
                    if noreply:
                        return True
